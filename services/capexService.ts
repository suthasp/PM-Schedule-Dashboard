...
/**
 * Fetch the published CAPEX 2026 sheet CSV (via the local API proxy). The sheet
 * is flat single-header-row like the Problem sheet, so it shares that
 * pipeline's transform and grid dataset shape.
 */
export async function fetchCapexData(): Promise<ProblemData> {
